...
class McpSettings : PersistentStateComponent<McpSettings.State> {
    ...
    data class State(
        ...
        var stdioEnabled: Boolean = false // Enable stdio transport in GUI mode (always on in headless)
    )
    ...
    var stdioEnabled: Boolean
        get() = state.stdioEnabled
        set(value) { state.stdioEnabled = value }
