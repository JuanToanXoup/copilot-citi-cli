...
class KtorMcpServer(
    ...
    fun start(): StartResult {
        // Pre-check port availability synchronously.
        // Ktor CIO binds the port asynchronously after start(wait=false) returns,
        // so a BindException would surface as an unhandled coroutine exception.
        try {
            ServerSocket(port, 0, InetAddress.getByName(host)).close()
        } catch (e: BindException) {
            LOG.warn("Port $port is already in use")
            return StartResult.PortInUse(port)
        } catch (e: Exception) {
            LOG.warn("Cannot bind to port $port: ${e.message}")
            return StartResult.Error(e.message ?: "Unknown error", e)
        }
        ...
        return try {
            ...
            LOG.warn("Failed to start MCP server: ${e.message}", e)
