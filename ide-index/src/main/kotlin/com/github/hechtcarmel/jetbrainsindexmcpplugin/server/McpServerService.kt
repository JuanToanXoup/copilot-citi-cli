...
class McpServerService : Disposable {
    ...
    private var stdioTransport: StdioMcpTransport? = null
    ...
    init {
        ...
        // Start stdio transport if enabled in settings (for GUI mode)
        // In headless mode, HeadlessMcpStarter handles stdio startup directly
        if (McpSettings.getInstance().stdioEnabled) {
            startStdioTransport()
        }
     ...
     * Can be called from GUI settings or programmatically from HeadlessMcpStarter.
     */
    fun startStdioTransport() {
        if (stdioTransport?.isRunning() == true) {
            LOG.warn("Stdio transport already running")
            return
        }
        LOG.info("Starting stdio MCP transport")
        stdioTransport = StdioMcpTransport(jsonRpcHandler, coroutineScope).also { it.start() }
    }

    /**
     * Stops the stdio MCP transport.
     */
    fun stopStdioTransport() {
        stdioTransport?.stop()
        stdioTransport = null
    }
     ...
     */
    fun isStdioRunning(): Boolean = stdioTransport?.isRunning() == true
...
data class ServerStatusInfo(
    ...
    val stdioRunning: Boolean = false
)
