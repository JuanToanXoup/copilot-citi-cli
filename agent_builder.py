...
STATIC_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "agent_builder")
...
class BuilderHandler(BaseHTTPRequestHandler):
    ...
    def _api_build(self, body):
        self._sse_start()
        try:
            from agent_export import build_agent
            config = body
    ...
    def _api_export_script(self, body):
        try:
            from agent_export import export_script
            config = body
            name = config.get("name", "agent").strip()
            output_dir = os.path.expanduser(f"~/.copilot-cli/builds/{name}")
            os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    import argparse
    p = argparse.ArgumentParser(description="Agent Builder web UI")
    p.add_argument("-p", "--port", type=int, default=8420)
