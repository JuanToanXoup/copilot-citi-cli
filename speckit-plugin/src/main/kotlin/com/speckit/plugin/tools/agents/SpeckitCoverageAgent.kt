        ...
        return buildString {
            ...
            when {
                ...
                testConventions == null -> {
                    ...
                    appendLine()
                    appendLine("**Steps:**")
                    appendLine("1. Check if a constitution already exists: call `speckit_read_memory` with name `constitution.md`")
                    appendLine("2. Read 2-3 existing test files from the project to learn the patterns:")
                    appendLine("   naming style, assertion library, mock patterns, test data setup, file organization")
                    appendLine("3. Document the conventions you found (naming, assertions, mocks, DI, test data, organization)")
                    appendLine("4. Save: call `speckit_write_memory` with name `test-conventions.md`")
                    appendLine()
